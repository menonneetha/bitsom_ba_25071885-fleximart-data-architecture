...
create_queries = [
    ...
    "CREATE TABLE customers (customer_id INT PRIMARY KEY AUTO_INCREMENT, first_name VARCHAR(50), last_name VARCHAR(50), email VARCHAR(100) UNIQUE, phone VARCHAR(20), city VARCHAR(50), registration_date DATE);",
    "CREATE TABLE products (product_id INT PRIMARY KEY AUTO_INCREMENT, product_name VARCHAR(100), category VARCHAR(50), price DECIMAL(10,2), stock_quantity INT);",
    "CREATE TABLE orders (order_id INT PRIMARY KEY AUTO_INCREMENT, customer_id_old VARCHAR(50), order_date DATE, total_amount DECIMAL(10,2), status VARCHAR(20) DEFAULT 'Pending');",
    """
    CREATE TABLE order_items (
        order_item_id INT PRIMARY KEY AUTO_INCREMENT, 
        customer_id_old VARCHAR(50), 
        product_name_old VARCHAR(100), 
        quantity INT, 
...
# 5. Calculate total_amount from subtotals
df_sales['total_amount'] = df_sales.groupby(['customer_id_old', 'order_date'])['subtotal'].transform('sum')
print(f"✅ Calculated total sales value: ${df_sales['total_amount'].sum():,.2f}")

# Final Selections
cust_f = df_customers[['first_name', 'last_name', 'email', 'phone', 'city', 'registration_date']]
prod_f = df_products[['product_name', 'category', 'price', 'stock_quantity']]
ord_f = df_sales[['customer_id_old', 'order_date', 'total_amount', 'status']]
item_f = df_sales[['customer_id_old', 'product_name_old', 'quantity', 'unit_price', 'subtotal']]
...
records_to_load = {
    ...
    'sales_raw.csv': len(ord_f) + len(item_f)
}
...
ord_f.to_sql('orders', con=engine, if_exists='append', index=False)
item_f.to_sql('order_items', con=engine, if_exists='append', index=False)

print("✅ SUCCESS: Full ETL pipeline finished! 4 tables loaded!")